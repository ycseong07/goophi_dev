...
# fitting xgboost
# fitted_boost_model <- parsnip::boost_tree(trees = 15) %>%
#  parsnip::set_engine("xgboost") %>%
#  parsnip::set_mode("classification") %>%
#  parsnip::fit(diabetes~., data = diabetes_train)

f <- "diabetes~."
fitted_boost_model <- goophi::xgboost(data = diabetes_train, formula = f)
...
result <-
  diabetes_test %>%
  dplyr::bind_cols(
    parsnip::predict.model_fit(fitted_boost_model, diabetes_test),
    parsnip::predict.model_fit(fitted_boost_model, diabetes_test, type = "prob")
  )
result
