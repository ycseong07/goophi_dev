...
randomForest_phi <- function(engine = "ranger",
                             mode = "classification"){
...
xgboost_phi <- function(engine = "xgboost",
  ...
  result <- parsnip::boost_tree(mtry = tune(), trees = tune(), min_n = tune(), tree_depth = tune(), learn_rate = tune(), loss_reduction = tune(), sample_size = tune()) %>%
    parsnip::set_engine(engine = engine) %>%
    parsnip::set_mode(mode = mode)
  return(result)
}
